...
router.post('/', (req: Request, res: Response) => {
  try {
    ...
    stmt.run(
      ...
      data.idType || 'national',
      data.hasMilitaryNumber !== false,
      data.rankAuthority || '',
      data.financialStatus || 'salary',
      data.connectionType || 'دائم',
      data.directSupervisor || 'commander',
      data.lastEditedBy || '',
      now,
      JSON.stringify(data.promotionHistory || []),
      JSON.stringify(data.movementHistory || []),
      JSON.stringify(data.gear || []),
      JSON.stringify(data.documents || []),
      JSON.stringify(data.commendations || []),
      JSON.stringify(data.disciplinaryRecords || []),
